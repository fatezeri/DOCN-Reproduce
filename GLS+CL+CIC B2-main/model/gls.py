...
class GlsBlock(nn.Module):
    ...
    def forward(self, x_in):
        ...
        ker = torch.randn([self.out_channel * nb, self.in_channel , k, k ], requires_grad = self.requires_grad, device = device)
        ...
        lmda=betadistribution.sample((nb, 1, 1, 1)).to(device)
...
class Gls(nn.Module):
    def __init__(self, alpha=1.0,glsmix_f=1,out_channel = 3, in_channel = 3, interm_channel = 2, scale_pool = [1, 3], n_layer = 4, out_norm = 'frob'):
        ...
        self.layers.append(
            GlsBlock(alpha=self.mixalpha,out_channel = self.interm_channel, in_channel = self.in_channel, scale_pool = scale_pool)
            )

        for ii in range(n_layer - 2):
            self.layers.append(
            GlsBlock(alpha=self.mixalpha,out_channel = self.interm_channel, in_channel = self.interm_channel, scale_pool = scale_pool)
            )

        self.layers.append(
            GlsBlock(alpha=self.mixalpha,out_channel = self.out_channel, in_channel = self.interm_channel, scale_pool = scale_pool, use_act = False)
            )
